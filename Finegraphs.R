...
ADR <- ggplot(Aldiv.abiotic, aes(x=S, y=Theta, colour=Richness))+ geom_point(size=5, alpha=.6, label=Aldiv.abiotic$Richness, position=position_dodge(), stat="identity", )+ scale_color_gradientn(colours=jet.colors(100), na.value="black", space="rgb", guide="colourbar")
ADR <- ADR+labs(x="S", y="Theta")
...
ADA <- ggplot(Aldiv.abiotic, aes(x =test2$Cellcount, y =Richness, colour=Theta))+ geom_point(size=5, alpha=.6, label=Aldiv.abiotic$Richness, position=position_dodge(), stat="identity", )+ scale_color_gradientn(colours=jet.colors(100), na.value="black", space="rgb", guide="colourbar")
...
DF <- ggplot(Aldiv.abiotic, aes(x = DfromF, y =test2$Cellcount, colour=Richness))+ geom_point(size=5, alpha=.6, label=Adiv.abiotic2$Evenness.SW, position=position_dodge(), stat="identity")+ scale_color_gradientn(colours=jet.colors(100), na.value="black", space="rgb", guide="colourbar")
...
#Kuroshio All
ALK <- ggplot(Aldiv.abiotic, aes(x =Richness, y = Evenness.SW, colour=Theta))+ geom_point(size=6, alpha=.6, label=Aldiv.abiotic$Theta, position=position_dodge(), stat="identity", )+ scale_color_gradientn(colours=jet.colors(7), space="rgb", guide="colourbar")
...
EU <- EU+theme(axis.text.x=element_text(size=20, vjust=0.5), axis.text.y=element_text(size=20, vjust=.05))
 EU+stat_smooth(method = "lm" , poly size = 1)
